...
class Instance(object):
    def __init__(self, tokenizer, sentence_pack, args):
        ...
        self.token_range = []
        self.megatron_tokens = tokenizer.encode(self.sentence)
        self.length = len(self.megatron_tokens)
        self.megatron_tokens_padding = torch.zeros(args.max_sequence_len).long()
        ...
        for i in range(self.length):
            self.megatron_tokens_padding[i] = self.megatron_tokens[i]
...
def load_data_instances(sentence_packs, args):
    instances = list()
    tokenizer = BertTokenizer.from_pretrained(args.megatron_tokenizer_path)
...
class DataIterator(object):
    ...
    def get_batch(self, index):
        ...
        for i in range(index * self.args.batch_size,
            ...
            megatron_tokens.append(self.instances[i].megatron_tokens_padding)
        ...
        megatron_tokens = torch.stack(megatron_tokens).to(self.args.device)
        ...
        return sentence_ids, megatron_tokens, lengths, masks, sens_lens, token_ranges, aspect_tags, tags
