...
from model import MultiInfermegatron
import utils
...
def train(args):
    ...
    model = MultiInfermegatron(args).to(args.device)

    optimizer = torch.optim.Adam([
        {'params': model.megatron.parameters(), 'lr': 1.0e-5},
    ...
    for i in range(args.epochs):
        ...
        if joint_f1 > best_joint_f1:
            model_path = args.model_dir + 'megatron' + args.task + '.pt'
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset', type=str, default="res14", choices=["res14", "lap14", "res15", "res16"],
                        help='dataset')
    parser.add_argument('--max_sequence_len', type=int, default=150,
                        help='max length of a sentence')
    parser.add_argument('--device', type=str, default="cuda",
                        help='gpu or cpu')

    parser.add_argument('--megatron_model_path', type=str,
                        default="pretrained/",
                        help='pretrained albert model path')
    parser.add_argument('--megatron_tokenizer_path', type=str,
                        default="nvidia/megatron-bert-uncased-345m",
                        help='pretrained bert tokenizer path')
    parser.add_argument('--megatron_feature_dim', type=int, default=1024,
