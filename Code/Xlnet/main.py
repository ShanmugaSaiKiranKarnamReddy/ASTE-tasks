...
def train(args):
    ...
    model = MultiInferxlnet(args).to(args.device)

    optimizer = torch.optim.Adam([
        {'params': model.xlnet.parameters(), 'lr': 1e-5},
    ...
    for i in range(args.epochs):
        ...
        if joint_f1 > best_joint_f1:
            model_path = args.model_dir + 'xlnet' + args.task + args.dataset+ str(args.nhops) +'.pt'
...
def test(args):
    ...
    model_path = args.model_dir + 'xlnet' + args.task + args.dataset + str('1') +'.pt'
    model = torch.load(model_path).to(args.device)
    model.eval()

    sentence_packs = json.load(open(args.prefix + args.dataset + '/test1.json'))
...
def predict(args):
    ...
    triples = []
    triplets_sentences = []
    sentence = "The lean linear palate shows dried sour cherry, licorice and coffee bean alongside assertive close-grained tannins."
    ...
    with open("/content/drive/MyDrive/GTS-main/data/wine226/test2.json", "w+") as test_json:
      json.dump(triplets_sentences,test_json )
    sentence_packs = json.load(open(args.prefix + args.dataset + '/test2.json'))
    instances = load_data_instances(sentence_packs, args)
