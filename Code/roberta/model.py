...
class MultiInferRoBerta(torch.nn.Module):
    def __init__(self, args):
        super(MultiInferRoBerta, self).__init__()
        ...
        self.tokenizer = RobertaTokenizer.from_pretrained(args.roberta_tokenizer_path)

        self.cls_linear = torch.nn.Linear(args.roberta_feature_dim*2, args.class_num)
        self.feature_linear = torch.nn.Linear(args.roberta_feature_dim*2 + args.class_num*3, args.roberta_feature_dim*2)
    ...
    def forward(self, tokens, masks):
        roberta_feature, _ = self.roberta(tokens, masks)
        roberta_feature = self.dropout_output(roberta_feature)

        roberta_feature = roberta_feature.unsqueeze(2).expand([-1, -1, self.args.max_sequence_len, -1])
        roberta_feature_T = roberta_feature.transpose(1, 2)
        features = torch.cat([roberta_feature, roberta_feature_T], dim=3)
