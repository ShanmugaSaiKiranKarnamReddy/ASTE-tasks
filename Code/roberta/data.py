...
class Instance(object):
    def __init__(self, tokenizer, sentence_pack, args):
        ...
        self.token_range = []
        self.roberta_tokens = tokenizer.encode(self.sentence)
        self.length = len(self.roberta_tokens)
        self.roberta_tokens_padding = torch.zeros(args.max_sequence_len).long()
        ...
        for i in range(self.length):
            self.roberta_tokens_padding[i] = self.roberta_tokens[i]
...
def load_data_instances(sentence_packs, args):
    instances = list()
    tokenizer = RobertaTokenizer.from_pretrained(args.roberta_tokenizer_path)
...
class DataIterator(object):
    ...
    def get_batch(self, index):
        ...
        for i in range(index * self.args.batch_size,
            ...
            roberta_tokens.append(self.instances[i].roberta_tokens_padding)
        ...
        roberta_tokens = torch.stack(roberta_tokens).to(self.args.device)
        ...
        return sentence_ids, roberta_tokens, lengths, masks, sens_lens, token_ranges, aspect_tags, tags
