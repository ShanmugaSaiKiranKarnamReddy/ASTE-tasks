...
class Instance(object):
    def __init__(self, tokenizer, sentence_pack, args):
        ...
        self.token_range = []
        self.distilbert_tokens = tokenizer.encode(self.sentence)
        self.length = len(self.distilbert_tokens)
        self.distilbert_tokens_padding = torch.zeros(args.max_sequence_len).long()
        ...
        for i in range(self.length):
            self.distilbert_tokens_padding[i] = self.distilbert_tokens[i]
        ...
        for i, w, in enumerate(self.tokens):
            ...
            length = self.length
            token_diff = self.token_range[-1][-1]
            value = length - token_diff
        assert self.length == self.token_range[-1][-1]+value
...
def load_data_instances(sentence_packs, args):
    instances = list()
    tokenizer = DistilBertTokenizer.from_pretrained(args.distilbert_tokenizer_path)
...
class DataIterator(object):
    ...
    def get_batch(self, index):
        ...
        for i in range(index * self.args.batch_size,
            ...
            distilbert_tokens.append(self.instances[i].distilbert_tokens_padding)
        ...
        distilbert_tokens = torch.stack(distilbert_tokens).to(self.args.device)
        ...
        return sentence_ids, distilbert_tokens, lengths, masks, sens_lens, token_ranges, aspect_tags, tags
