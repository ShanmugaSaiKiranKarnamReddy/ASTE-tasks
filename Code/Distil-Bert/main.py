...
from model import MultiInferdistilBert
import utils
...
def train(args):
    ...
    model = MultiInferdistilBert(args).to(args.device)

    optimizer = torch.optim.Adam([
        {'params': model.distilbert.parameters(), 'lr': 5e-5},
    ...
    for i in range(args.epochs):
        ...
        if joint_f1 > best_joint_f1:
            model_path = args.model_dir + 'distilbert' + args.task + '.pt'
...
if __name__ == '__main__':
    ...
    parser.add_argument('--distilbert_model_path', type=str,
                        default="pretrained/",
                        help='pretrained distilbert model path')
    parser.add_argument('--distilbert_tokenizer_path', type=str,
                        default="distilbert-base-uncased",
                        help='pretrained bert tokenizer path')
    parser.add_argument('--distilbert_feature_dim', type=int, default=768,
