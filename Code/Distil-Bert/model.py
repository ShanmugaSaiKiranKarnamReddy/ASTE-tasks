...
class MultiInferdistilBert(torch.nn.Module):
    def __init__(self, args):
        super(MultiInferdistilBert, self).__init__()

        self.args = args
        self.config = DistilBertConfig.from_pretrained( 'distilbert-base-uncased', output_hidden_states=True)
        self.distilbert = DistilBertModel.from_pretrained(args.distilbert_model_path,config= self.config)
        self.tokenizer = DistilBertTokenizer.from_pretrained(args.distilbert_tokenizer_path)

        self.cls_linear = torch.nn.Linear(args.distilbert_feature_dim*2, args.class_num)
        self.feature_linear = torch.nn.Linear(args.distilbert_feature_dim*2 + args.class_num*3, args.distilbert_feature_dim*2)
    ...
    def forward(self, tokens, masks):
        distilbert_feature,_ = self.distilbert(tokens, masks)
        distilbert_feature = self.dropout_output(distilbert_feature)

        distilbert_feature = distilbert_feature.unsqueeze(2).expand([-1, -1, self.args.max_sequence_len, -1])
        distilbert_feature_T = distilbert_feature.transpose(1, 2)
        features = torch.cat([distilbert_feature, distilbert_feature_T], dim=3)
